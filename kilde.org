...
#+begin_src bigquery
  ...
  WITH
    ...
    UR AS (
    SELECT CONCAT(FORMAT_DATE('%d. %h', LAG(dato) OVER(PARTITION BY userOrProfile ORDER BY dato)), ' - ', FORMAT_DATE('%d. %h', dato)) periode,
           *,
...
#+begin_src bigquery
  ...
  WITH
    ...
    UR AS (
    SELECT CONCAT(FORMAT_DATE('%d. %h', LAG(dato) OVER(PARTITION BY userOrProfile, lojalitet ORDER BY dato)), ' - ', FORMAT_DATE('%d. %h', dato)) periode,
           *,
           ROUND(`Kommer og drar` / `Tot brukere`, 3) Andel
      FROM GL)
